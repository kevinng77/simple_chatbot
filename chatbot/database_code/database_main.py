...
class DataBase:
    ...
    def database_return(self, content, intent, DST):
        ...
        if query_domain == '景点':
            ...
            try:
                sub_data = self.database[query_domain]
                target = DST[query_domain + '-' + query_slot[:3]]
                if isinstance(target,list):
                    target = target[0]
                candidates = sub_data[target]['地铁']
            except:
            ...
            if candidates is None:
                return target + '附近没有地铁站'
            else:
                return target + '从' +candidates+'出来'
